    ...
    while (1) {
        ...
        if (choice == 1) {
            ...
            if (cust != 0) {
                ...
                int year, milage, vehicle_choice;
                view.promptVehicleChoice(vehicle_choice);                
                view.promptVehicleInfo(make, model, colour, year, milage);
                
                if(vehicle_choice == 1)
                    (*cust) += vf.create(make, model, colour, year, milage);
                else if(vehicle_choice == 2) {
                    int na;
                    view.promptNumAxles(na);
                    (*cust) += vf.create(make, model, colour, year, milage, na);
                } else {
                    bool sc;
                    view.promptSideCar(sc);
                    (*cust) += vf.create(make, model, colour, year, milage, sc);
                }
            } else {
                view.displayInvalid();
            }
...
void ShopController::initShop() {
    ...
    newCustomer = new Customer("Maurice", "Mooney", "2600 Colonel By Dr.", 
                                        "(613)728-9568");
    newVehicle = vf.create("Ford", "Fiesta", "Red", 2007, 100000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Ford", "F150", "Black", 2017, 1000, 3);
    (*newCustomer) += newVehicle;
    ...
    newCustomer = new Customer("Abigail", "Atwood", "43 Carling Dr.", 
                                        "(613)345-6743");
    newVehicle = vf.create("Subaru", "Forester", "Green", 2016, 40000);
    (*newCustomer) += newVehicle;
    mechanicShop += newCustomer;
    ...
    newCustomer = new Customer("Brook", "Banding", "1 Bayshore Dr.", 
                                        "(613)123-7456");
    newVehicle = vf.create("Honda", "Accord", "White", 2018, 5000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Volkswagon", "Beetle", "White", 1972, 5000);
    (*newCustomer) += newVehicle;
    mechanicShop += newCustomer;
    ...
    newCustomer = new Customer("Ethan", "Esser", "245 Rideau St.", 
                                        "(613)234-9677");
    newVehicle = vf.create("Toyota", "Camery", "Black", 2010, 50000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Mack", "Granite", "Red", 2000, 500000, 8);
    (*newCustomer) += newVehicle;
    mechanicShop += newCustomer;
    ...
    newCustomer = new Customer("Eve", "Engram", "75 Bronson Ave.", 
                                        "(613)456-2345");
    newVehicle = vf.create("Toyota", "Corolla", "Green", 2013, 80000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Toyota", "Rav4", "Gold", 2015, 20000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Toyota", "Prius", "Blue", 2017, 10000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Kawasaki", "Ninja", "Blue", 2018, 100, false);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Kawasaki", "Z650", "Grey", 2014, 5000, false);
    (*newCustomer) += newVehicle;
    mechanicShop += newCustomer;
    ...
    newCustomer = new Customer("Victor", "Vanvalkenburg", "425 O'Connor St.", 
                                        "(613)432-7622");
    newVehicle = vf.create("GM", "Envoy", "Purple", 2012, 60000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("GM", "Escalade", "Black", 2016, 40000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("GM", "Malibu", "Red", 2015, 20000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("GM", "Trailblazer", "Orange", 2012, 90000);
    (*newCustomer) += newVehicle;
    newVehicle = vf.create("Harley", "Touring", "Black", 2006, 8000, true);
    (*newCustomer) += newVehicle;
    mechanicShop += newCustomer;
